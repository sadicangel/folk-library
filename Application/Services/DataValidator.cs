...
internal sealed class DataValidator : IDataValidator
{
    ...
    public async Task ValidateAsync()
    {
        ...
        _logger.LogInformation("Albums...: {albumCount}", artists.SelectMany(a => a.Albums).DistinctBy(a => a.Id).Count());
        _logger.LogInformation("Tracks...: {trackCount}", artists.SelectMany(a => a.Albums.SelectMany(b => b.Tracks)).DistinctBy(c => c.Id).Count());
