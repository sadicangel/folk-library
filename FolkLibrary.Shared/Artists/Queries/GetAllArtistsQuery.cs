...
public sealed class GetAllArtistsQuery : IRequest<Page<ArtistDocument>>
{
    ...
    public sealed class Specification : Specification<ArtistDocument>
    {
    ...
    public sealed class Handler : IRequestHandler<GetAllArtistsQuery, Page<ArtistDocument>>
    {
        ...
        public async Task<Page<ArtistDocument>> Handle(GetAllArtistsQuery request, CancellationToken cancellationToken)
        {
            var specification = new Specification(request.QueryParams);
            var artists = await _artistDocumentRepository.ListPagedAsync(specification, request.ContinuationToken, cancellationToken);
            return artists;
        }
    }
}
