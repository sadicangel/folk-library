...
internal abstract class MongoRepository<TId, TDocument> : IRepository<TDocument>
    where TDocument : Document<TId>
    where TId : IId<TId>
{
    ...
    public string CollectionName { get; } = typeof(TDocument).Name[..^8].Pluralize().Camelize();
    ...
    public async Task<Page<TDocument>> ListPagedAsync(ISpecification<TDocument> specification, string? continuationToken, CancellationToken cancellationToken = default)
    {

        TryDecodePageIndex(continuationToken, out int pageIndex);

        var countFacet = AggregateFacet.Create(CountName, PipelineDefinition<TDocument, AggregateCountResult>.Create(
            new IPipelineStageDefinition[]
            {
                PipelineStageDefinitionBuilder.Count<TDocument>()
            }));

        var dataFacet = AggregateFacet.Create(DataName, PipelineDefinition<TDocument, TDocument>.Create(
            new IPipelineStageDefinition[]
            {
        ...
        var data = aggregate.Facets.Single(f => f.Name == DataName).Output<TDocument>();

        var nextPageIndex = pageIndex + 1;

        return new Page<TDocument>
        {
    ...
    public async Task<Page<TResult>> ListPagedAsync<TResult>(ISpecification<TDocument, TResult> specification, string? continuationToken, CancellationToken cancellationToken = default)
    {
        TryDecodePageIndex(continuationToken, out int pageIndex);
        ...
        var dataFacet = AggregateFacet.Create(DataName, PipelineDefinition<TDocument, TResult>.Create(
            new IPipelineStageDefinition[]
            {
                PipelineStageDefinitionBuilder.Sort(specification.ToSortDefinition()),
                PipelineStageDefinitionBuilder.Skip<TDocument>(pageIndex * PageSize),
